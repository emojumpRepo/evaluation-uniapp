...
function viewDetail(record: IQuestionnaireResultList) {
  ...
  // 这里可以跳转到详情页面
  uni.navigateTo({
    url: `/pages-sub/record/record-detail?id=${record.questionnaireId}&time=${record.completedTime}`,
  })
}
...
onMounted(async () => {
  ...
  if (props.babyId && props.questionnaireId) {
    const res = await getHistoryQuestionnaireResult({ questionnaireId: props.questionnaireId, babyId: props.babyId })
    const { code, data } = res
...
<template>
  <view class="mx-auto mt-4 w-screen bg-gray-50">
    <!-- 记录列表 -->
    <view class="mx-4 pb-4 space-y-3">
      <view
        ...
        hover-class="bg-gray-50" @tap="viewDetail(record)"
      >
        <!-- 标题和时间 -->
        <view class="mb-4">
